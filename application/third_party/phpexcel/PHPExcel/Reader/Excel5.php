...
//		echo '<br />';
	...
	private function _readFont()
	{
		...
		if (!$this->_readDataOnly) {
			...
			$underlineType = ord($recordData[$2]);
	...
	private function _readXf()
	{
		...
		if (!$this->_readDataOnly) {
			...
			$horAlign = (0x07 & ord($recordData[$2])) >> 0;
			switch ($horAlign) {
				case 0:
			...
			$wrapText = (0x08 & ord($recordData[$2])) >> 3;
			switch ($wrapText) {
				case 0:
			...
			$vertAlign = (0x70 & ord($recordData[$2])) >> 4;
			switch ($vertAlign) {
				case 0:
			...
			if ($this->_version == self::XLS_BIFF8) {
				...
				// offset:  8; size: 1; Indentation, shrink to cell size, and text direction
					...
					$indent = (0x0F & ord($recordData[$2])) >> 0;
					$objStyle->getAlignment()->setIndent($indent);

					// bit: 4; mask: 0x10; 1 = shrink content to fit into cell
					$shrinkToFit = (0x10 & ord($recordData[$2])) >> 4;
					switch ($shrinkToFit) {
						case 0:
				...
				$orientationAndFlags = ord($recordData[$2]);
			...
			while ($offset < $length) {
				...
				switch ($extType) {
					case 4:		// fill start color
						...
						if ($xclfType == 2) {
							$rgb = sprintf('%02X%02X%02X', ord($xclrValue[$2]), ord($xclrValue[$2]), ord($xclrValue[$2]));
	...
	private function _readFormula()
	{
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			if ( (ord($recordData[$2]) == 0)
				&& (ord($recordData[$2]) == 255)
				&& (ord($recordData[$2]) == 255) ) {
			...
			} elseif ((ord($recordData[$2]) == 1)
				&& (ord($recordData[$2]) == 255)
				&& (ord($recordData[$2]) == 255)) {

				// Boolean formula. Result is in +2; 0=false, 1=true
				$dataType = PHPExcel_Cell_DataType::TYPE_BOOL;
				$value = (bool) ord($recordData[$2]);

			} elseif ((ord($recordData[$2]) == 2)
				&& (ord($recordData[$2]) == 255)
				&& (ord($recordData[$2]) == 255)) {

				// Error formula. Error code is in +2
				$dataType = PHPExcel_Cell_DataType::TYPE_ERROR;
				$value = self::_mapErrorCode(ord($recordData[$2]));

			} elseif ((ord($recordData[$2]) == 3)
				&& (ord($recordData[$2]) == 255)
				&& (ord($recordData[$2]) == 255)) {
	...
	private function _readBoolErr()
	{
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			$boolErr = ord($recordData[$2]);

			// offset: 7; size: 1; 0=boolean; 1=error
			$isError = ord($recordData[$2]);
